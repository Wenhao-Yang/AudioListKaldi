...
# options for vox1
parser.add_argument('--dataset-dir', type=str, default='/home/yangwenhao/dataset/CN-Celeb',
                    help='path to dataset')
parser.add_argument('--dataset-dir-2', type=str, default='/home/yangwenhao/dataset/CN-Celeb2',
                    help='path to dataset')
parser.add_argument('--skip-v1', action='store_true', default=False, help='just prepare v2')
...
if __name__ == '__main__':
    ...
    if not args.skip_v1:
        ...
        try:
            ...
            for spk in spks_dir:
                ...
                utts = [x for x in spk.iterdir() if x.is_file() and x.suffix in wav_suffix]
                for utt in utts:
                    utt_dic = {}
                    uid = spk.name + '-' + utt.name.rstrip(utt.suffix)
                    utt_dic['uid'] = uid
        ...
        for spk in dev_lst_f.readlines():
            ...
            for utt in cn_lst:
                if utt['spk'] == spk_name:
                    if 'flac' in utt['path']:
                        this_path = 'sox -t flac ' + utt['path'] + ' -t wav -r 16k -b 16 - channels 1 |'
                    else:
                        this_path = utt['path']
                    wav_scp.append(utt['uid'] + ' ' + this_path + '\n')
        ...
        for idx, utt_pair in enumerate(enroll):
            ...
            path_flac = path.replace('wav', 'flac')
            if os.path.exists(path_flac.rstrip('\n')):
                path = 'sox -t flac ' + path_flac.rstrip('\n') + ' -t wav -r 16k -b 16 - channels 1 |\n'
            elif not os.path.exists(path.rstrip('\n')):
                raise ValueError(path, path_flac)
    ...
    for data_dir in data_dirs:
        ...
        if data_dir_path.exists():
            ...
            print("  %s does not exist." % str(data_dir_path))
    ...
    if len(cn_lst) > 0:
        ...
        print('\tThere are %d wav files in dev2' % len(wav_scp))
